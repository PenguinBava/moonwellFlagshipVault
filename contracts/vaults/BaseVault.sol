...
abstract contract BaseVault is Governable, Storage, PausableUpgradeable, ReentrancyGuardUpgradeable, ERC4626Upgradeable {
    ...
    function totalAssets() public view virtual override returns (uint256) {
        uint256 underlying = available();
        return underlying + balanceOfPool();
    }

    // it calculates how much 'want' the strategy has working in the farm.
    function balanceOfPool() public view returns (uint256) {
        uint256 stakingShare = stakingContract.balanceOf(address(this));

        return stakingContract.previewRedeem(stakingShare);
    }
